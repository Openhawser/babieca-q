defmodule Core.Client do
  ...
  def create_topic(topic_name) do
    GenServer.call({:global, :BabiecaQ}, {:create_topic, topic_name})
  end

  @spec add_message_2_topic(String.t(), String.t()) :: {:ok | :error, String.t()}
  def add_message_2_topic(msg, topic_name) do
    GenServer.call({:global, :BabiecaQ}, {:add_message, topic_name, msg})
  end
  ...
  def add_multiples_messages_2_topic(messages, topic_name) do
    GenServer.call({:global, :BabiecaQ}, {:add_multiples_messages, topic_name, messages})
  end

  @spec delete_topic(String.t()) :: {:ok | :error, String.t()}
  def delete_topic(topic_name) do
    GenServer.call({:global, :BabiecaQ}, {:delete_topic, topic_name})
  end

  @spec create_user(String.t(), String.t()) :: {:error, String.t()} | {:ok, any()}
  def create_user(user_name, topic_name) do
    GenServer.call({:global, :BabiecaQ}, {:create_user, user_name, topic_name})
  end

  @spec consumer_pull(String.t(), String.t()) :: {:error, String.t()} | {:finished, String.t()} | {:ok, any()}
  def consumer_pull(user_name, topic_name) do
    case GenServer.call({:global, :BabiecaQ}, {:get_next_message, user_name, topic_name}) do
      {:error, value} -> Logger.error(value)
                         {:error, value}
      {:ok, value} -> case GenServer.call({:global, :BabiecaQ}, {:move_user_to_next_message, user_name, topic_name}) do
                        :ok -> {:ok, value}
  ...
  def delete_messages_of_topic(topic_name) do
    GenServer.call({:global, :BabiecaQ}, {:delete_messages, topic_name})
  end
end